...
type MorphologyKind =
  ...
  | 'suffix'
  | 'unknown';
type SegmentKind = MorphologyKind | 'table';
type DecodeSource =
  | 'builtin-compact'
  | 'builtin-robinson'
  | 'builtin-bsb'
...
type RobinsonValues = Partial<Record<MorphologyKind, Record<string, LocalizedLabel>>>;
...
interface MorphologyDecodeResult {
  rawCode: string;
  source: DecodeSource;
...
interface ResolveMorphologyParams {
...
interface PreferredLanguageParams {
  uiLanguage?: string | null;
  dictLanguage?: string | null;
}

interface MorphologyMeaningRow {
  indication: string;
  applicableTo: string;
  language: string;
  meaning: string;
...
interface MorphologyTopicRow {
  topic: string | null;
}

const SUPPORTED_LANGS: readonly SupportedLang[] = ['en', 'pt', 'es'];

const BUILTIN_LANGUAGE: Record<SupportedLang, LanguageStrings> = {
  pt: {
...
const ROBINSON_PARTS: Record<string, 'partOfSpeech'> = {
  A: 'partOfSpeech',
  C: 'partOfSpeech',
  D: 'partOfSpeech',
  I: 'partOfSpeech',
...
const ROBINSON_VALUES: RobinsonValues = {
  partOfSpeech: {
...
function normalizeBuiltInLang(lang: string | null | undefined): SupportedLang {
  const normalized = String(lang || DEFAULT_LANG).trim().toLowerCase().slice(0, 2);
  return isSupportedLang(normalized) ? normalized : DEFAULT_LANG;
}

function getLangStrings(lang: string | null | undefined): LanguageStrings {
...
function parseMorphologyMeaning(
  rawCode: string | null | undefined,
  lang: string | null | undefined
): MorphologyDecodeResult | null {
  return (
...
function queryMorphologyMeaning(
  ...
  return (
    (db.prepare(sql).get(...params) as MorphologyMeaningRow | undefined) || null
  );
}
