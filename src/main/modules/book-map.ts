...
const _grapheToIndex = new Map<number, number>();
for (let i = 0; i < GRAPHE_BOOK_NUMBERS.length; i++) {
  _grapheToIndex.set(GRAPHE_BOOK_NUMBERS[i], i);
}

function twBookToGraphe(bi: number): number | undefined {
  return GRAPHE_BOOK_NUMBERS[bi - 1];
}

function grapheToTwBook(bookNumber: number): number {
 ...
 */
function buildVerseIndex(isNtOnly: boolean): Map<number, Map<number, VerseIndexEntry>> {
  const index = new Map<number, Map<number, VerseIndexEntry>>();
  let line = 0;
