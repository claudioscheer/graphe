...
import * as thewordBible from '../theword-bible-provider.ts';
import * as thewordTwm from '../theword-twm-provider.ts';
import { twBookToGraphe } from '../book-map.ts';
import {
...
type TheWordTwmHandle = NonNullable<ReturnType<typeof thewordTwm.load>>;
...
interface CommentaryBatchEntry {
  bookNumber: number;
  chapterFrom: number;
  verseFrom: number;
 ...
 */
function convertTagsToMyBible(line: string): string {
  let result = line;
 ...
 */
async function convertCommentaryType2(
  ...
  try {
    ...
    const batch: CommentaryBatchEntry[] = [];
 ...
 */
async function convertCommentaryType3(
  handle: TheWordTwmHandle,
  inputPath: string,
  outputDir: string,
  onProgress?: ProgressCallback
    ...
    db.transaction(() => {
      for (const [bookNumber, topicIds] of books) {
        const texts = topicIds
          .map((id: number) => thewordTwm.extractPlainText(handle, id))
...
export { convert, convertTagsToMyBible };

if (typeof module !== 'undefined') module.exports = { convert, convertTagsToMyBible };
