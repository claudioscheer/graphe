...
import * as myswordProvider from '../mysword-provider.ts';
import { BOOK_NAMES, GRAPHE_BOOK_NUMBERS, twBookToGraphe } from '../book-map.ts';
import {
  normalizeStrongNumber,
  sanitizeStrongTags,
  sanitizeSupportedTags,
} from './tag-sanitizer.ts';

type ProgressCallback = (current: number, total: number) => void;

interface MySwordVerseRow {
  Book: number;
  Chapter: number;
  Verse: number;
  text: string | null;
...
interface MySwordDictionaryEntryRow {
  word: string;
  data: string | null;
  lexeme?: string | null;
}

function normalizeConvertedVerse(text: string | null | undefined): string {
  return String(text || '')
    .replace(/\r/g, '')
    .replace(/\u00a0/g, ' ')
    .replace(/<[^>]+>/g, ' ')
 ...
 *   - <br> → line break
 */
function convertMySwordTags(text: string | null | undefined): string {
  let result = String(text || '');

  // 1. Convert Strong's with optional morphology+lemma: <WH/G####><WTmorph l="lemma">
  result = result.replace(
    /<W([HG])(\d+)>(?:<WT([^ >]*?)(?:\s+l="([^"]*)")?>)?/gi,
    (_match: string, _prefix: string, number: string, morph: string, lemma: string): string => {
  ...
  result = result.replace(/「([\s\S]*?)」/g, (match: string, inner: string): string => {
  ...
  result = result.replace(/<(HEB|TRA|SEP|ACF|SBL|GRC)>([^<]*)<\/\1>/gi, (
    _match: string,
    _tag: string,
    inner: string
  ): string => {
...
function getEffectiveExtension(filePath: string): string {
...
function convertBible(
  ...
  try {
    ...
    const insertedBooks = new Set<number>();
    let inserted = 0;
    let nonEmpty = 0;
...
function convertDictionary(
  ...
  try {
    ...
    const entries = handle.db
      ...
      .all() as MySwordDictionaryEntryRow[];
...
export { convert, convertMySwordTags };

if (typeof module !== 'undefined') module.exports = { convert, convertMySwordTags };
