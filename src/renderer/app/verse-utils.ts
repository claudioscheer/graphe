 ...
 */
interface StrongNumberToken {
  prefix: string;
  number: string;
}

interface VerseUtilsApi {
  cleanText(text: string | null | undefined): string;
  cleanTextWithStrongs(text: string | null | undefined, strongNumbers: StrongNumberToken[]): string;
}
...
export const VerseUtils = (() => {
   ...
   */
  function cleanTextWithStrongs(
    text: string | null | undefined,
    strongNumbers: StrongNumberToken[]
  ): string {
    if (!text) return '';
    let s = String(text);
    ...
    s = s.replace(/(\S+)\s*<S>([\s\S]*?)<\/S>/gi, (_match: string, word: string, code: string) => {
