...
function decorateNativeSelect(selectEl: HTMLSelectElement | null): void {
...
const Settings: SettingsApiInstance = (() => {
...
window.showTooltip = function (paneId: string, message: string): void {
...
function copySelectedVerses(paneId: string = PaneManager.getActivePaneId()): boolean {
...
function openNavigationForActivePane(): void {
...
async function openSearchResult(result: SearchNavigationResult | null): Promise<void> {
  if (!result) return;
  const location = {
...
document.addEventListener('click', (e: MouseEvent) => {
  ...
  if (commentaryRef) {
    ...
    const paneId = PaneManager.getActivePaneId();
    const targetPaneId = PaneManager.getNavigationTarget(paneId);
    PaneManager.navigatePane(targetPaneId, bookNumber, chapter, verse)
      .then((ok) => {
        if (!ok) window.showTooltip?.(targetPaneId, I18n.t('refUnavailable'));
      })
...
function showUpdateBanner({ version, url }: UpdateInfo): void {
